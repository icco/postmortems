...
title: Incident Post-Mortem and Security Advisory — Data Exposure After travis-ci.com Outage (Mar 2018)
...
Reasons captured in the retrospective: developer environments could connect to the production primary with write access because the read-only follower was harder to reach (the easy path was the dangerous path); the team rushed back to user-facing recovery and inadvertently left applications running while the database was empty, creating the duplicate-id situation; certain alerts weren't re-armed after the outage, hiding the broken cron scheduler. The total data loss was ~15 minutes once the database was restored. Remediations include making it harder to connect to production with write access from a developer machine, treating "applications still running while the database is unreachable" as an explicit incident state, and improving alert re-arming as part of the standard recovery checklist.
